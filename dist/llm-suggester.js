 ...
 */
async function generateSuggestions(apiKey, model, prTitle, prBody, diff, docsContext, promptInstruction = '', commentIntro = '') {
    ...
    const contextText = docsContext.length === 0
        ...
        : docsContext.map((doc, i) => `[${i + 1}] ${doc.file} (lines ${doc.startLine}-${doc.endLine}) - score: ${doc.score.toFixed(3)}\n${doc.text}`).join('\n\n');
    const userPrompt = `
PR Title: ${prTitle}
